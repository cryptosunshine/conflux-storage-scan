...
async function resolveFileInfo(
	...
	target: StorageDownloadTarget,
): Promise<StorageNodeFileInfo> {
	const info =
		"txSeq" in target
			? await client.getFileInfoByTxSeq(target.txSeq)
			: await client.getFileInfo(target.root, true)
	if (!info || info.pruned) {
		throw new StoragePocError("DOWNLOAD_NOT_FOUND", "File is not available on the selected Storage Node")
	}
	if (
		"root" in target &&
		info.tx.dataMerkleRoot.toLowerCase() !== target.root.toLowerCase()
	) {
		throw new StoragePocError(
			"INTEGRITY_MISMATCH",
			"Storage Node FileInfo Root does not match the requested Root",
		)
	}
	return info
}
...
async function compareFiles(
	...
	for (let offset = 0; offset < left.size; offset += comparisonBytes) {
		...
		if (
			leftView.length !== rightView.length ||
			leftView.some((byte, index) => byte !== rightView[index])
		) {
			return false
		}
	}
...
export async function downloadAndVerifyStorageFile({
	...
	for (let segmentIndex = 0; segmentIndex < segmentCount; segmentIndex += 1) {
		...
		const encoded = await client.downloadSegmentByTxSeq(
			info.tx.seq,
			startChunk,
			endChunk,
		)
		if (!encoded) {
			throw new StoragePocError(
				"DOWNLOAD_FAILED",
				`Storage Node returned no data for Segment ${segmentIndex}`,
			)
		}
		try {
			...
			throw new StoragePocError(
				"DOWNLOAD_FAILED",
				`Storage Node returned invalid Base64 for Segment ${segmentIndex}`,
				{ cause },
			)
		}
	}
	...
	if (downloaded.size !== info.tx.size) {
		throw new StoragePocError(
			"DOWNLOAD_FAILED",
			"Storage Node returned fewer bytes than FileInfo declares",
		)
	}
	...
	if (
		prepared.root.toLowerCase() !==
		info.tx.dataMerkleRoot.toLowerCase()
	) {
		throw new StoragePocError(
			"INTEGRITY_MISMATCH",
			"Downloaded file Merkle Root does not match Storage Node FileInfo",
		)
	}

	const bytesEqual = originalFile
		? await compareFiles(file, originalFile)
		: undefined
	if (bytesEqual === false) {
		throw new StoragePocError(
			"INTEGRITY_MISMATCH",
			"Downloaded bytes do not match the original file",
		)
	}
